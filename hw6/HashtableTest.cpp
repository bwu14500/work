...
int main(int argc, char** argv) {
    ...
    // uncomment for indexing argument (true for indexing false for no indexing)
    // see the commented out reportAll() function below
//    int index;
//    if(argc >= 4) {
//        index = (argv[3][0] == '0')? false : true;
//    }
    ...
    string s = "abcd";
    addCombos(letters, s, 3, ht);

    ofstream oFile((argc >= 2)? argv[1] : "output.txt");
    oFile << "probe type: " << probeType << endl;
    ht.reportAll(oFile);
    // I made another method for indexing (not required for the assignment, but may be useful for comparison)
    // ht.reportAll(oFile, index);
    oFile.close();
    return 0;
}